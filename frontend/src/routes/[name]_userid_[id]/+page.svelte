<script lang="ts">
    ...
    import { page, onMount, extractNameAndIdFromPath, loadInfo } from '$lib/libraries'
    ...
    async function loadAllInfo(ids: number[], type: string) {
        let infos;
        infos = await Promise.all(ids.map(id => loadInfo(id.toString(), '_playlist', '/' + type + '/info/')));
        infos.sort((a, b) => parseFloat(a.id.toString()) - parseFloat(b.id.toString()));
        return infos
    }

    onMount(async() => {
        userInfo = await loadInfo(id, name, '/auth/get_user/');
